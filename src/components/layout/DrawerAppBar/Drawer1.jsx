import agencyKineticsLogo from '../../../Images/agencyMail.png'
...
const useStyles = makeStyles((theme) => ({
    drawer: {
      width: (props) => (props.open ? 200 : 60), // Width when open vs mini variant
      flexShrink: 0,
      whiteSpace: 'nowrap',
    ...
    drawerPaper: {
      width: (props) => (props.open ? 200 : 60), // Adjust paper width based on drawer state
      overflowX: 'hidden', // Ensures nothing overflows when in mini variant
      transition: theme.transitions.create('width', {
        easing: theme.transitions.easing.sharp,
        duration: theme.transitions.duration.enteringScreen,
      }),
    ...
    drawerHeader: {
      ...
      // Adjust alignment for mini variant
      justifyContent: (props) => (props.open ? 'space-around' : 'center'),
      paddingLeft: 0, // Remove left padding
    paddingRight: 0, // Remove right padding
    [theme.breakpoints.up('sm')]: {
      paddingLeft: 0, // Remove left padding on larger screens
      paddingRight: 0, // Remove right padding on larger screens
    },
    },
    drawerBrandingText: {
      fontSize: "1.25em",
      fontWeight: 600,
      userSelect: "none",
      opacity: (props) => (props.open ? 1 : 0), // Hide text in mini variant
    ...
    drawerListItem: {
      ...
      cursor: "pointer",
      display: 'flex',
      justifyContent: 'center', // Keep icons centered regardless of open/close state
      paddingLeft: (props) => (props.open ? theme.spacing(4) : theme.spacing(6)),
    //   marginLeft: (props) => (props.open ? 0 : theme.spacing(1.5)), // Add marginLeft to compensate in mini variant
      transition: 'all 0.3s ease-in-out',
      paddingTop: theme.spacing(0.5),  // Reduced top padding
  paddingBottom: theme.spacing(0.5), 
    },
    listSubheader: {
      marginTop: "1em",
      opacity: (props) => (props.open ? 1 : 0), // Hide subheader in mini variant
    ...
    selectedText: {
      color: 'rgb(127, 86, 217)',
      opacity: (props) => (props.open ? 1 : 0), // Hide text when in mini variant
      transition: 'opacity 0.3s ease-in-out',
    },
    bottomInfoContainer: {
      marginTop: 'auto',
    ...
    infoText: {
      textAlign: 'center',
      marginBottom: theme.spacing(0), // Adds space between daysLeft and workspace_name
    },
  }));
...
const DrawerComponent = ({ open }) => {
  ...
  // console.log(location.pathname.includes("task"))
  
  useEffect(() => {
  if (location.pathname.includes("payments-and-invoices-details")) {
    setSelectedItem("");
  } else if (location.pathname.includes("subscription-plans") || location.pathname.includes("profile")) {
    setSelectedItem("");
  } else if (location.pathname.includes("quotes") || location.pathname.includes("quote")) {
    ...
    setSelectedItem("Orders");
  } else if (location.pathname.includes("chats") || location.pathname.includes("chat")) {
    setSelectedItem("Chats");
  } else if (location.pathname.includes("onboard") || location.pathname.includes("onboards")) {
  ...
  const renderChats = (
    <React.Fragment>
      ...
      >
        ...
        <ListItemIcon className={selectedItem === 'Chats' ? classes.selectedIcon : ''}>
        <ChatIcon className={selectedItem === 'Chats' ? classes.selectedIcon : ''}/>
        </ListItemIcon>
        {open && (
            <ListItemText
          primary="Chats"
          className={selectedItem === 'Chats' ? classes.selectedText : ''}
        />
        )}
      ...
      {open && (
        <React.Fragment>
          {/* {superadmin && (
            ...
            <ListItemText primary={`${daysLeft} days left`} className={classes.infoText} />
          </ListItem>
          )} */}
          
          <ListItem className={classes.drawerListItem}>
            <ListItemText primary={workspace_name} className={classes.infoText} />
          </ListItem>
        </React.Fragment>
      )}
    </Box>
  ...
  const renderIntegration = (
    <React.Fragment>
      ...
      >
        ...
        <ListItemIcon className={selectedItem === 'Integrations' ? classes.selectedIcon : ''}>
        <WebhookIcon className={selectedItem === 'Integrations' ? classes.selectedIcon : ''}/>
        </ListItemIcon>
        {open && (
            <ListItemText
          primary="Integrations"
          className={selectedItem === 'Integrations' ? classes.selectedText : ''}
        />
        )}
  ...
  const renderWorkspaceName = (
    <React.Fragment>
        ...
        <ListItemText style={{marginLeft:'40px'}} primary={`${workspace_name}`} />
      </ListItem>
    )}
    </React.Fragment>
  ...
  return (
    ...
    >
      ...
      {/* {userRole === 'SUPERADMIN' && renderChats} */}
      {userRole === 'SUPERADMIN' && renderIntegration}
      {userRole === 'SUPERADMIN' && renderInfo}
      {/* {userRole === 'SUPERADMIN' && renderDaysLeft}
...
{/* project manager */}
      ...
      {userRole === 'PROJECTMANAGER' && renderInfo}
...
{/* client */}
      ...
      {/* {userRole === 'CLIENT' && renderChats} */}
      {userRole === 'CLIENT' && renderInfo}
