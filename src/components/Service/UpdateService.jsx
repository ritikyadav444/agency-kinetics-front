...
import { Box, Slider,Autocomplete, Tooltip, Button, FormControl, Grid, Input, InputLabel, MenuItem, Select, TextField, InputAdornment } from '@mui/material';
...
const UpdateService = ({handleUpdateClose, match, selectedServiceId}) => {
  ...
  const {loading, updateError, isUpdated} = useSelector((state)=>state.serviceDU)
  ...
  const [service_desc , setService_desc] = useState(service ? service.service_desc : "");
  ...
  const service_cover_img_filename = service && service.service_cover_img ? service.service_cover_img.split('/').pop().split('_')[0] : ''
  ...
  useEffect(() => {
    if(service && service._id !==serviceId){
        ...
        setSliderValue({
          min: service.service_amount?.min || 1, 
          max: service.service_amount?.max || 100000 
        });
  ...
  }, [dispatch, error, navigate, isUpdated, updateError, service, serviceId]);
  ...
  const createServiceSubmitHandler = async (e) => {
    ...
    const serviceData = {
      ...
      const response = await dispatch(updateService( serviceId,serviceData));
      if (response.success) {
      ...
      dispatch(getService())
      dispatch(createNotification(combined.user._id, `Service ${service.service_name} Updated Successfully`, routeLink));
      if (combined.user.role !== 'SUPERADMIN'){
        // console.log("INHEFERE", superAdminId)
        dispatch(createNotification(superAdminId?._id, `Service ${service.service_name} Updated Successfully By ${combined.user.fname} ${combined.user.lname}`, routeLink));
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
                ...
                <Grid item xs={12} sm={12}>
                  ...
                  >
                    <Tooltip title={service_cover_img ? "Image uploaded" : "Upload"} placement="top">
                    <span style={{color:'white'}}>
                        {service_cover_img
                          ? "Uploaded Image " + service_cover_img_filename
                          : "Upload Image"}
                      </span>
